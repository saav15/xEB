...
public class XEBTinkersTraitHandler {
    ...
    public static void onLivingHurt(LivingHurtEvent event) {
        ...
        if (event.getEntity() instanceof Player player) {
            ...
            int spikedLvl = getModifierLevel(held, "xeb:spiked_conditioning");
            if (spikedLvl > 0) {
        ...
        if (event.getSource().getEntity() instanceof Player attacker) {
            ...
            // Flaming
            int flamingLvl = getModifierLevel(held, "tconstruct:fiery");
            if (flamingLvl > 0) {
                event.getEntity().setSecondsOnFire(4 * flamingLvl);
            }
            
            // Sandy
            int sandyLvl = getModifierLevel(held, "tconstruct:blinding");
            if (sandyLvl > 0) {
            ...
            // Depressing
            int depLvl = getModifierLevel(held, "tconstruct:weakening");
            if (depLvl > 0) {
            ...
            // Slightly Depressing
            int sdepLvl = getModifierLevel(held, "tconstruct:weakening");
            if (sdepLvl > 0) {
                event.getEntity().addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SLOWDOWN, 60, 0));
            }

            // Sticky
            int stickyLvl = getModifierLevel(held, "tconstruct:seared");
            if (stickyLvl > 0) {
        ...
        if (event.getEntity() instanceof Player victim) {
            ...
            // Spiky
            int spikyLvl = getModifierLevel(held, "tconstruct:thorns");
            ...
            // Protected
            int protLvl = getModifierLevel(held, "tconstruct:shield");
            ...
            // Shielded
            int shieldLvl = getModifierLevel(held, "tconstruct:shield");
            if (shieldLvl > 0) {
            ...
            // Reactive
            int reactLvl = getModifierLevel(held, "tconstruct:knockback");
            if (reactLvl > 0) {
                boolean isProjectile = event.getSource().getDirectEntity() instanceof Projectile;
            ...
            // Undying (Cheat Death)
            int undyingLvl = getModifierLevel(held, "tconstruct:life_leech");
    ...
    public static void onPlayerTick(TickEvent.PlayerTickEvent event) {
        if (event.phase == TickEvent.Phase.END && !event.player.level().isClientSide) {
            ...
            int reflexesLvl = getModifierLevel(held, "xeb:elite_reflexes");
    ...
    public static void onEliteSlayerDamage(LivingDamageEvent event) {
        if (event.getSource().getEntity() instanceof Player player) {
            ...
            // Elite Slayer Trait
            int slayerLvl = getModifierLevel(held, "xeb:elite_slayer");
            if (slayerLvl > 0) {
            ...
            // Damaging Essence
            int dmgLvl = getModifierLevel(held, "tconstruct:sharpness");
            if (dmgLvl > 0) {
                event.setAmount(event.getAmount() * (1f + 0.15f * dmgLvl));
            }

            // Twin Essence
            int twinLvl = getModifierLevel(held, "tconstruct:overslime");
